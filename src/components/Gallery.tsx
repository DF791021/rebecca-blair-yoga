      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-12">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-light text-gray-800">Journey Through Practice</h3>
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className="flex items-center space-x-2 px-4 py-2 bg-white shadow-lg rounded-full hover:shadow-xl transition-all duration-300 transform hover:scale-105"
            >
          ...
          <div className="scroll-container flex space-x-6 overflow-x-auto scrollbar-hide pb-4" style={{ scrollBehavior: 'auto' }}>
            {[...images, ...images, ...images].map((image, index) => (
              <div
                key={index}
                className="flex-shrink-0 w-72 h-96 rounded-2xl overflow-hidden cursor-pointer group relative transform transition-all duration-500 hover:scale-105 hover:shadow-2xl"
        ...
        <div className="mt-20 text-center">
          <div className="max-w-3xl mx-auto bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-8 transform transition-all duration-500 hover:scale-105">
            <blockquote className="text-2xl font-light text-gray-800 italic mb-4">
              "Yoga is not about touching your toes. It is about what you learn on the way down."
            </blockquote>
            <p className="text-gray-600">— Judith Hanson Lasater</p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-95 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="relative max-w-4xl max-h-full">
            <button
              ...
              className="absolute -top-10 right-0 text-white hover:text-gray-300 transition-all duration-300 transform hover:scale-110"
            >
              <X size={32} />
            </button>
            <img
              ...
              className="max-w-full max-h-full object-contain rounded-lg transform transition-transform duration-500 hover:scale-105"
            />
          </div>
        </div>
      )}
